...
- **`docs/apexcharts_power_day_card.yaml`** / **`swipe`**: Y-Achse Watt **`min: auto`** (Export nach unten); **Netz**-Serie `group_by: avg`; Wallboxen auf **rechter Achse kW**. **`pvoptimizer_helpers.yaml`**: Abzug Wallbox **kW** → W per `unit_of_measurement`. **`ENERGY_DASHBOARD_ENTITIES.md`**: Troubleshooting + Template angepasst.
